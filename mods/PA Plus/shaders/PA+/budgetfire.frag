...
float noise(in vec3 x) {
    ...
    vec2 rg = textureLod(flamedis, (uv+ 0.5)/256.0, 0.0).yx;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    fragColor = flixel_texture2D(bitmap, openfl_TextureCoordv) + vec4(col, texture(iChannel0, fragCoord / iResolution.xy).a);
}
