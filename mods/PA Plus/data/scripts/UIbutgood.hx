...
var ratingStuff:Array<Dynamic> = [
    ['dawg', 0.2], //From 0% to 19%
    ['poo', 0.4], //From 20% to 39%
    ['peeing', 0.5], //From 40% to 49%
    ['Vineboom', 0.6], //From 50% to 59%
    ['Bruh', 0.69], //From 60% to 68%
    ['Ok but get better', 0.7], //69%
    ['Good but not good enought', 0.8], //From 70% to 79%
    ['Great idk what to say', 0.9], //From 80% to 89%
    ['Sick AF!', 1], //From 90% to 99%
    ['OUTDATED MEME', 1] //The value on this one isn't used actually, since Perfect is always "1"
];
...
public var fontShit:String = "menuFont";
...
function create() {
    timeTxt = new FlxText(0, 20, 400, "X:XX - X:XX", 22);
...
function update(elapsed:Float) {
    if (curStep >= 1){
        timeTxt.text = CoolUtil.timeToStr(Conductor.songPosition).split('.')[0];
    ...
    if (songScore > 0 || acc > 0 || misses > 0)
        hudTxt.text = "Score: " + songScore + " | Skill Issues: " + misses +  " | USELESS STAT: " + ratingFC + " (" + acc + "%)";
}
...
function onPlayerHit(event) {
    ...
    hudTxt.angle = FlxG.random.float(-13.5,13.5);
    hudTxtTween = FlxTween.tween(hudTxt, {angle: 0}, 0.2, {onComplete: function(twn:FlxTween) {hudTxtTween = null;}});

    hudTxt.scale.y = FlxG.random.float(2.085,2.045);
    hudTxt.scale.x = FlxG.random.float(2.085,2.045);
